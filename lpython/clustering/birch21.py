...
for k, v in count:
	...
	indexes = [i for i,x in enumerate(birch_model.labels_) if x == k]
	for i in indexes:
		noise_points.append(X[i])
   indexes = [i for i,x in enumerate(birch_model.labels_) if x != k]
	for i in indexes:
		data_points.append(X[i])
...
#print("Labels=",labels)
unique_labels = set(birch_model.labels_)
